...
-  **Claude Code**
優等生。バランスが良く、コーディングエージェントの様々な知見をいち早く盛り込む。コードの精度が高い。基本間違いなし。
SkillやHook、スラッシュコマンド、サブエージェントなど、機能が多い。


- **Gemini CLI**
無料で使える範囲が大きい。
コンテキストウィンドウがで大きい。（100万トークン）
...
- 単体テストの有効性とコードの凝集度の話 **（テスト）**
- ドメイン駆動設計の解説とメリット **（設計）**
- 実践的なSOLID原則 **（設計）**
- Reactの基礎とNext.jsのキャッシュ戦略 **（フロント）**
- AWS・GCインフラ構成ノック **（インフラ）**
- Webサーバー・メールサーバー構築ハンズオン **（インフラ）**
- ログインの実装ハンズオン **（認証）**
- Webアプリケーション開発で知るべき基礎的注意点 **（セキュリティ）**